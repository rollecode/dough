...
// Who is asking. The session route passes its signed-in user; the API passes the user the key or
// token belongs to. Everything below only needs these three fields.
...
// Dougie's answer, context and all. Both the web page and /api/v1/chat go through here, so the
...
export async function respondToChat(
  ...
  const { messages, image, image_media_type, add_expense } = input;
  const originUrl = origin?.url ?? "http://localhost";
  const originCookie = origin?.headers.get("cookie") ?? "";
      ...
      try {
        ...
        if (cached) {
          ...
          if (cacheAge > 2 * 60 * 60 * 1000) {
            ...
            fetch(new URL("/api/ynab/sync", originUrl).toString(), {
              method: "POST",
              headers: { cookie: originCookie },
        ...
        context = {
          ...
          locale,
          householdProfile,
          currentUser: user.display_name || user.email || "unknown",
        };
      ...
      if (transactions.length > 0) {
        ...
        if (defaultAcc) {
          ...
          for (const tx of transactions) {
            ...
            try {
              const txRes = await fetch(new URL("/api/ynab/transaction", originUrl).toString(), {
                method: "POST",
                headers: {
