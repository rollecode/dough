...
// Everything the dashboard shows, in one call: the daily budget and why it
...
// Shared by /api/v1/dashboard and the web's /api/dashboard-model.
export function dashboardFor(userId: number) {
  const db = getDb();
  const now = new Date();
  ...
  const linkedAccountIds = (
    db.prepare("SELECT ynab_account_id FROM user_linked_accounts WHERE user_id = ?").all(userId) as {
      ynab_account_id: string;
    }[]
  ).map((row) => row.ynab_account_id);
  const displayName =
    (db.prepare("SELECT COALESCE(display_name, '') AS name FROM users WHERE id = ?").get(userId) as
      | { name: string }
      | undefined)?.name ?? "";
  const personalBudgetShare =
    (db.prepare("SELECT COALESCE(budget_share, 0) AS share FROM users WHERE id = ?").get(userId) as
      | { share: number }
      | undefined)?.share ?? 0;
